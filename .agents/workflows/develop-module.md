...
Request → [Middleware chain] → Controller → Service → Repository
                                   │            │
                                   │            └─ throw ErrorResponse (business errors)
                                   │
...
- **Service**: ALL business validation and error throwing happens here. Use `ErrorResponse` subclasses (`BAD_REQUEST`, `NOT_FOUND`, `CONFLICT`, `UNAUTHORIZED`, `FORBIDDEN`). NEVER return `null`/`false` to indicate failure — always `throw`.
- **Middleware**: Handles cross-cutting concerns (auth, validation, rate limiting, upload). NEVER contains business logic. Errors go through `next(new ErrorResponse(...))`.
...
### 2. Repository (if new query needed) — `src/repositories/x.repository.js`
...
### 3. Controller — `src/controllers/x.controller.js`

**Controller is a thin adapter: extract → call → respond.** That's it.

- Extract data from `req` (body, params, query)
- Call ONE Service method
- Send response with `new OK({...}).send(res)` or `new CREATED({...}).send(res)`
...
async create(req, res) {
  try {
    const result = await xService.create(req.body);
    if (!result) throw new BAD_REQUEST({ message: "Failed" });
    new CREATED({ data: result }).send(res);
  } catch (err) { ... }
}
```

### 4. Validation — `src/validations/x.validation.js`
...
- [ ] ALL error cases are handled in **Service** via `throw new ErrorResponse(...)`
...
- [ ] Response uses `new OK()/CREATED().send(res)`, NOT `res.status().json()`

...
```
src/jobs/x.job.js          →  Job definition (calls Service)
...
- **New domain?** → Create a new `src/jobs/x.job.js` file

### 2. Create / Update Job File — `src/jobs/x.job.js`
...
### 3. Implement Service Method (if not exists) — `src/services/x.service.js`
...
```js
// ── Import jobs ──────────────────────────────────────────────────────────────
import xJob from "../../jobs/x.job.js";
